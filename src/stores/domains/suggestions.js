...
export const useSuggestionsStore = defineStore('suggestions', () => {
    ...
    function upsertSuggestionFromEntry(entry) {
        ...
        if (existing) {
            Object.assign(existing, withLegacyDensityFields(payload))
        ...
        foodSuggestions.value.push({
            ...
            sectionUsage: {},
            ...withLegacyDensityFields(payload)
        })
    }
    ...
    function addSuggestion(suggestion) {
        ...
        if (!name || !Number.isFinite(caloriesValue)) return
        foodSuggestions.value.push(withLegacyDensityFields({
            id: `suggestion_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
            name,
    ...
    function updateSuggestion(id, updates) {
        ...
        if (index === -1) return
        foodSuggestions.value[index] = withLegacyDensityFields({
